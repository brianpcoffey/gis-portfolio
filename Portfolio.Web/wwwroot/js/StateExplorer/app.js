...
async function initialize() {
    ...
    MapManager.onMapClick(async (event) => {
        try {
            ...
            if (!MapManager.view || !MapManager.view.hitTest) return;
            ...
            if (!graphic) return;
            const fid = String(graphic.attributes.featureId);
            const state = getState();
            ...
            console.error("Map click handler error", e);
        }
    });
    ...
    MapManager.onMapHover((event) => {
        try {
            ...
            MapManager.view.hitTest(event)
                .then(res => {
                    const graphic = res.results?.find(r =>
                        r.graphic &&
                        r.graphic.attributes &&
                        r.graphic.attributes.featureId
                    )?.graphic;
                    if (graphic) {
                        MapManager.highlightFeature(String(graphic.attributes.featureId));
                    } else {
                        MapManager.highlightFeature(null);
                    }
                })
                .catch(e => console.error("hover hitTest", e));
        } catch (e) {
            console.error("Hover handler error", e);
        }
    });

    // UI Events
    document.addEventListener("ui:zoomSaved", e => {
        MapManager.zoomToFeature(e.detail.featureId);
    });

    document.addEventListener("ui:removeSaved", async e => {
        ...
        try {
            ...
            UI.showToast("Remove", "Saved feature removed", "success");
        } catch { }
    });

    document.addEventListener("ui:selectSaved", e => {
        setSelectedFeature(String(e.detail.featureId));
    });

    document.addEventListener("ui:selectCollection", e => {
        setState({
            selectedCollectionId: e.detail.collectionId ? String(e.detail.collectionId) : null
        });
    });

    document.addEventListener("ui:openSaveModal", e => {
        const fid = String(e.detail.featureId);
        const feature = e.detail.feature || getState().features.find(f =>
            String(f.featureId || f.FeatureId) === fid
        );
        const name = feature?.displayName || feature?.Name || feature?.name || `State ${fid}`;
        UI.openSaveModal(fid, name);
    });

    document.addEventListener("ui:confirmSave", async e => {
        const { featureId, description, collectionId } = e.detail;
        const state = getState();
        const feature = state.features.find(f =>
            String(f.featureId || f.FeatureId) === String(featureId)
        );
        if (!feature) {
            UI.showToast("Error", "Feature not found", "danger");
            return;
        }
        const btn = document.getElementById("btnConfirmSave");
        if (btn) btn.disabled = true;
        try {
            const saved = await FeatureService.saveFeature(feature, description, collectionId);
            const normalized = {
                ...saved,
                featureId: String(
                    saved.featureId ||
            ...
            UI.showToast("Saved", "Feature saved successfully", "success");
            UI.closeSaveModal();
        } catch (err) {
    ...
    document.addEventListener("ui:search", e => {
        ...
        const match = state.features.find(f =>
            (f.displayName || f.Name || f.name || "").toLowerCase().includes(q)
        );
        if (match) {
            const id = String(match.featureId || match.FeatureId);
            setSelectedFeature(id);
            MapManager.zoomToFeature(id);
        } else {
            UI.showToast("Search", "No matching state found", "primary");
        }
    });
    ...
    subscribe(state => {
        ...
        const feature = state.features.find(f =>
            String(f.featureId || f.FeatureId) === String(selectedId)
        );
        UI.renderFeatureDetails(feature || null);
    });
