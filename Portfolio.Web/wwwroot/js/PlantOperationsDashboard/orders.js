...
window.showOrderModal = function (orderId) {
    ...
    if (isEdit) {
        // Fetch order details
        window.apiFetch(`${window.PortfolioApi.routes.fiber.orders}/${orderId}`)
            .then(r => r.json())
            .then(data => {
                order = data;
...
<div class="modal fade" id="${modalId}" tabindex="-1" aria-labelledby="${modalId}Label" aria-modal="true" role="dialog">
  <div class="modal-dialog modal-lg">
    ...
    $('#orderForm').on('submit', function (e) {
        ...
        let url = isEdit ? `${window.PortfolioApi.routes.fiber.orders}/${order.id}` : window.PortfolioApi.routes.fiber.orders;
        window.apiFetch(url, {
            method: method,
